...
# Import plotly components separately to avoid potential import errors
try:
...
# Load data - Fixed the missing colon after elif use_sample_data
if uploaded_file is not None:
    try:
        ...
        st.stop()
elif use_sample_data:  # Added the missing colon here
...
# Create time series plot with Plotly if available
if plotly_available:
    fig = px.line(
        agg_data, 
        x=date_col, 
    ...
    for product in selected_products:
        ...
        ax.plot(product_data[date_col], product_data[volume_col], marker='o', label=product)
    ax.set_xlabel('Date')
    ax.set_ylabel('Volume')
    ax.set_title(f"Product Volume Trends ({time_granularity})")
    ax.legend()
    ax.grid(True)
    st.pyplot(fig)
...
with col1:
    ...
    if plotly_available:
        ...
        product_totals = agg_data.groupby(product_col)[volume_col].sum()
        fig, ax = plt.subplots(figsize=(8, 8))
        ax.pie(product_totals, labels=product_totals.index, autopct='%1.1f%%')
        ax.set_title("Volume Share by Product")
        st.pyplot(fig)
...
with col2:
    ...
    if plotly_available:
        ...
        # Fallback to matplotlib
        fig, ax = plt.subplots(figsize=(10, 6))
        sns.boxplot(x=product_col, y=volume_col, data=agg_data, ax=ax)
        ax.set_title("Volume Distribution by Product")
        st.pyplot(fig)
...
if time_granularity in ["Daily", "Weekly", "Monthly"]:
    ...
    if plotly_available:
        ...
        ax.set_title("Volume Heatmap by Month and Year")
        st.pyplot(fig)
...
if selected_features:
    ...
    with col1:
        ...
        if plotly_available:
            ...
            sns.barplot(x='Importance', y='Feature', data=feature_imp.head(15), ax=ax)
            ax.set_title("Top 15 Features Impacting Volume")
            st.pyplot(fig)
